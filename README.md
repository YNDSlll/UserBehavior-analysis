...
原始文件约 3.4 GB，没有提交到 GitHub。请从阿里天池下载数据集 `UserBehavior.csv`，放到 `data/raw/`，然后按以下顺序运行：
...
5. `notebooks/M4_流失预测.ipynb` 或 `notebooks/M4_流失预测建模.py`：生成特征表和模型结果。
...
原始数据目录被 `.gitignore` 忽略，避免误提交大文件；样例数据、处理后数据和看板预计算数据已保留，便于直接查看项目效果。

## 目录结构
```
UserBehavior/
├── app.py        Streamlit 可视化看板入口
├── guides/       个人学习资料（本地使用，不上传 GitHub）
├── data/
...
└── output/
    ├── charts/     图表
    ├── reports/    分析报告（M2_EDA结论、M4_建模结果、项目总结报告）
    └── app_data/   看板预计算数据（kpis/daily/hourly/funnel/cluster_profile/feature_importance/model_metrics）
```
...
| M4 | `notebooks/M4_流失预测.ipynb`、`notebooks/M4_流失预测建模.py` | 流失定义与预测建模 |
...
- [x] M5 报告 + 可视化（完成 Streamlit 看板、项目总结报告、面试话术和预计算数据）
...
- M5：完成预计算看板数据、Streamlit 可视化应用、项目总结报告和面试话术；看板入口为 `app.py`。
